...
func Login(id string, name string) string {
	...
	for _, siswa := range Students {
		dataSiswa := strings.Split(siswa, "_")
		if dataSiswa[0] == id && dataSiswa[1] == name {
			return "Login berhasil: " + dataSiswa[1]
		}
	}
...
func GetStudyProgram(code string) string {
	data, exist := StudentStudyPrograms[code]
	if exist == false {
...
func ModifyStudent(programStudi, nama string, fn studentModifier) string {
	// validasi apakah program studi dan nama kosong
	if programStudi == "" || nama == "" {
		return "ID, Name or Major is undefined!"
	}

	for i:=0;i<len(Students);i++{
		dataSiswa := strings.Split(Students[i], "_")
		if dataSiswa[1] == nama {
...
func UpdateStudyProgram(programStudi string, students *string) {
	dataSiswa := strings.Split(*students, "_")
	*students = dataSiswa[0] + "_" + dataSiswa[1] + "_" + programStudi
}
